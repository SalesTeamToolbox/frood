...
- [x] **TIER-01**: Composite performance score calculated from existing effectiveness data (success_rate, volume, speed with configurable weights)
...
- [x] **TIER-04**: Tier cached in memory with TTL — never computed on the routing hot path
- [x] **TIER-05**: Tier persisted to file for restart recovery — cache warmed from file on startup
...
- [x] **DASH-03**: Rewards system toggle switch with confirmation dialog in settings
...
- [x] **TEST-01**: Unit tests for score calculation logic (composite weights, edge cases, zero data)
