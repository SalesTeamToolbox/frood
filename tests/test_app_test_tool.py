...
class TestVisualCriticExtraction:
    """Tests for _extract_screenshot_b64 in iteration engine."""

    def test_no_screenshot_in_records(self):
        from agents.iteration_engine import IterationEngine, ToolCallRecord

        records = [
            ToolCallRecord(tool_name="shell", arguments={}, result="OK", success=True),
        ]
        result = IterationEngine._extract_screenshot_b64(records)
        assert result is None

    def test_screenshot_path_extraction(self, tmp_path):
        from agents.iteration_engine import IterationEngine, ToolCallRecord
        ...
        records = [
            ToolCallRecord(
                tool_name="app_test",
                arguments={"action": "smoke_test"},
                result=f"Screenshot: {png_path}\n\nVisual Analysis: Looks good",
                success=True,
            ),
        ]
        result = IterationEngine._extract_screenshot_b64(records)
        # Should succeed if Pillow is installed, None otherwise
    ...
    def test_nonexistent_screenshot_path(self):
        from agents.iteration_engine import IterationEngine, ToolCallRecord
        ...
        records = [
            ToolCallRecord(
                tool_name="app_test",
                arguments={},
                result="Screenshot: /nonexistent/path/screenshot.png",
                success=True,
            ),
        ]
        result = IterationEngine._extract_screenshot_b64(records)
    ...
    def test_non_app_test_tool_ignored(self):
        from agents.iteration_engine import IterationEngine, ToolCallRecord
        ...
        records = [
            ToolCallRecord(
                tool_name="shell",
                arguments={},
                result="Screenshot: /some/path.png",
                success=True,
            ),
        ]
        result = IterationEngine._extract_screenshot_b64(records)
